...
void worker_process_manager_init() {
	...
	bp_jobobject_pool = sharedbuffpool_create(sizeof(union wpcmd_job_data_types), OBJECT_POOL_SIZE);
	fprintf(stdout, "\tcreated bp_jobobject_pool with chunk size of %lu\n", sizeof(union wpcmd_job_data_types));
	bp_job_bigstringpool = sharedbuffpool_create(MAX_WOOF_EL_SIZE, OBJECT_POOL_SIZE);
...
int main(int argc, char **argv)
{
	...
	*/
	...
	ulfius_add_endpoint_by_val(&instance, "POST", "/2015-03-31/", "/s3/:bucket/:key", 0, &callback_s3_put, NULL);
